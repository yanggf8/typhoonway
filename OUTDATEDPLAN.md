...
>
> Canonical source of truth: **PROPOSAL.md**. A new implementation plan will be written once the proposal settles.
...
Goal: Compile to `typhoon.wasm` and run on wasmtime. Cloudflare Workers and browser are out of scope (see PROPOSAL §No-Go).
...
Native builds call the `libsql` crate directly. The wasmtime build calls `db-exec`, `db-query`, and atomic `db-batch`; the wasmtime host backs them with a local libSQL connection and runs `db-batch` inside `BEGIN IMMEDIATE … COMMIT`.
