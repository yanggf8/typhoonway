...
- **Dream is a single-writer batch coordinated through a `dream_runs` lease row.** Heartbeat-bearing; phase ∈ `{starting, light, rem, deep, prune, completing, succeeded, failed, cancelled, timed_out}`. Plain `typhoon dream` while a live run exists prints status (TTY) or exits non-zero (non-TTY); stale runs (heartbeat older than `dream.heartbeat_timeout_seconds`) are taken over. Cancellation is cooperative via `typhoon dream cancel [--wait]`: dream observes the request at the next phase boundary, persists work it has already paid for, and exits with status `cancelled`. Total runtime bounded by `dream.max_runtime_minutes`. ETA from EWMA over recent successful runs' phase durations, with a static fallback table for early runs.
